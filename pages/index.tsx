...
const Home: NextPage = () => {
  ...
  const denumire_produs = (nume_produs) => (
    <span className="badge bg-success">{nume_produs}</span>
  );
  const denumire_magazin = (nume_magazin) => (
    <div className="p-3 mb-2 bg-success text-white">{nume_magazin}</div>
  );
  ...
  // magazine[x].titlu = 'Mega image'
  // magazine[x].produse = ["Carne", "Lapte", "Branza", "Faina", "Sampon", "Banane"]

  const la_piata = (shopping) => (
    <div>{magazin(magazine[shopping].titlu, magazine[shopping].produse)}</div>
  );
